...
if(_chance >= 85) then { hint "The cashier hit the silent alarm, police has been alerted!"; [[0,format["ALARM! - Shop: %1 is being robbed!", _shop]],"life_fnc_broadcast",west,false] spawn life_fnc_MP; }; //We set a 15% chance that the silent alarm is being triggered, wich sends a 911-message to the police. 
...
if(_rip) then
{
	...
	if!(alive _robber) exitWith {};
	[[0,format["911 - Shop: %2 was just robbed by %1 for a total of $%3",name _robber, _shop, [_kassa] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
